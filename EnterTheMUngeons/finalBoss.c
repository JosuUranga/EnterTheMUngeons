...
void finalAtakeakSortu(SDL_Rect player, SDL_Rect bala) {
	if (EnemyV[0].FPSCounter > 50&&boss.healthPoints[4]>100) {
		if (final.finalHasBeenCreated[0] == 0 && final.counter[0] == 0) {
			...
			final.deltax = ((player).x + (player.w / 2)) - (boss.bossRect[4].x + boss.bossRect[4].w / 2);
			final.deltay = ((player).y + (player.h / 2)) - (boss.bossRect[4].y + boss.bossRect[4].h / 2);
			final.distance = sqrt(final.deltax*final.deltax + final.deltay * final.deltay);
		}
...
void finalAtakeakMugitu(SDL_Rect dest, SDL_Rect bala) {
	if (EnemyV[0].FPSCounter > 50) {
		if (boss.healthPoints[4] > 100) {
			...
			if (final.finalHasBeenCreated[0] == 1) {
				...
				final.finalRect[0].y += final.finalVelY[0];
				final.finalRect[0].x += final.finalVelX[0];
			...
			if (final.finalHasBeenCreated[1] == 1) {
				...
				final.finalRect[1].y += final.finalVelY[1];
				final.finalRect[1].x += final.finalVelX[1];
				if (!lifestruct.invencivilty) {
			...
			final.finalRect[2].y += final.finalVelY[2];
			final.finalRect[2].x += final.finalVelX[2];
			final.finalVelX[3] = 0;
			final.finalVelY[3] = ATAQUEVEL;
			final.finalRect[3].y += final.finalVelY[3];
			final.finalRect[3].x += final.finalVelX[3];
			final.finalVelX[4] = -ATAQUEVEL;
			final.finalVelY[4] = ATAQUEVEL;
			final.finalRect[4].y += final.finalVelY[4];
			final.finalRect[4].x += final.finalVelX[4];
			final.finalVelX[5] = -ATAQUEVEL;
			final.finalVelY[5] = 0;
			final.finalRect[5].y += final.finalVelY[5];
			final.finalRect[5].x += final.finalVelX[5];
			final.finalVelX[6] = -ATAQUEVEL;
			final.finalVelY[6] = -ATAQUEVEL;
			final.finalRect[6].y += final.finalVelY[6];
			final.finalRect[6].x += final.finalVelX[6];
			final.finalVelX[7] = 0;
			final.finalVelY[7] = -ATAQUEVEL;
			final.finalRect[7].y += final.finalVelY[7];
			final.finalRect[7].x += final.finalVelX[7];
	...
	if (collisioncheck(bala, boss.bossRect[4])) {
		...
		if(boss.healthPoints[4] > 200)final.health[0] -= 1.18;
		if (boss.healthPoints[4] > 100 && boss.healthPoints[4] <= 200)final.health[1] -= 1.18;
		if (boss.healthPoints[4] <= 100)final.health[2] -= 1.18;
